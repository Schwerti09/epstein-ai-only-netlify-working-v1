    ...
    <div class="container">
      <div class="brand">
        <div class="logo">
          ...
          <small>Wir bündeln öffentlich zugängliche Quellen und liefern KI‑Leseführung. Keine Volltexte, kein Hosting fremder Dokumente.</small>
        </div>
        <div class="badge">
...
async function generate(){
  ...
  const chips = unique.slice(0,10).map(q => `<a class="chip" href="/q/${slugify(q)}/">${esc(q)}</a>`).join("");
  const home = chrome(`
    <div class="grid">
      <section class="card pad">
        <div class="kicker">Breaking Index</div>
        <div class="h1">Finde Muster. Nicht nur Schlagzeilen.</div>
        <p class="p muted">Gib einen Namen/Begriff ein. Wir zeigen dir öffentliche Quellen (Preview) und liefern KI‑Leseführung. Premium entsperrt Deep‑Summary.</p>
        <div class="hr"></div>
        <div id="search">
  ...
  write(path.join(outDir,"impressum.html"), pageShell({
    ...
    body:chrome(`<section class="card pad"><div class="kicker">Impressum</div><div class="h1" style="font-size:34px">Angaben gemäß § 5 TMG</div><div class="hr"></div><div class="p"><strong>${esc(legal.company)}</strong><br>Inhaber: ${esc(legal.owner)}<br>Anschrift: ${esc(legal.address)}, ${esc(legal.city)}<br>E‑Mail: <a class="btn" href="mailto:${esc(legal.email)}">${esc(legal.email)}</a><br>Steuerliche Angaben: ${esc(legal.tax)}</div><div class="hr"></div><div class="p muted"><strong>Affiliate‑Hinweis:</strong> Diese Seite kann Affiliate‑Links enthalten.</div></section>`)
  }));
  write(path.join(outDir,"datenschutz.html"), pageShell({
    ...
    body:chrome(`<section class="card pad"><div class="kicker">Datenschutz</div><div class="h1" style="font-size:34px">Datenschutzerklärung</div><p class="p muted">Kurzfassung: Zahlungen via Stripe. KI‑Anfragen via Google Gemini. Wir speichern keine Inhalte dauerhaft serverseitig.</p><div class="hr"></div><div class="p">Verantwortlich: ${esc(legal.company)}, ${esc(legal.owner)}, ${esc(legal.address)}, ${esc(legal.city)} · ${esc(legal.email)}</div></section>`)
  }));
  write(path.join(outDir,"agb.html"), pageShell({
    ...
    body:chrome(`<section class="card pad"><div class="kicker">AGB</div><div class="h1" style="font-size:34px">Allgemeine Geschäftsbedingungen</div><p class="p muted">Digitaler Service (KI‑Leseführung). Keine Volltexte, nur Index + Links. Premiumzugang nach Zahlung.</p></section>`)
  }));
  ...
  for (const q of unique){
    ...
    urls.append if False else None
    urls.push(`/q/${slug}/`);
  }
...
Sitemap: /sitemap.xml
`);
  write(path.join(outDir,"opensearch.xml"), `<?xml version="1.0" encoding="UTF-8"?>
